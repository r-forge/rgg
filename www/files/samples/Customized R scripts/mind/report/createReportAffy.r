...
createReportAffy = function(data.eSet,data.normalized.eSet,rgghome,reportfolder=paste(rgghome,"reportoutput","/"),target,channel,bgCorrMethod,normMethod,pmcorrmethod,summarymethod,heatcov,boxlabels="colnames",pcncomp=3,pcgroup="none",maplots=TRUE){
	source(paste(rgghome,"report","boxplotSingle.r",sep="/"))
	source(paste(rgghome,"report","plotMASingle.r",sep="/"))
	source(paste(rgghome,"report","plotmeanSd.r",sep="/"))
	source(paste(rgghome,"report","doHeatmap.r",sep="/"))
	source(paste(rgghome,"report","prcompPlot.r",sep="/"))
	dir.create(reportfolder)
	file.copy(paste(rgghome,"report","style.css",sep="/"),paste(reportfolder,"style.css",sep="/"), overwrite = T)
  nrppi = 72
  w=8
  h=8
  wd = w*nrppi
  ...
  for (i in 1:ncol(exprs(data.eSet))){
    ...
    #dev.off()
  	...
  	##### LATER [ilhami]
	pdf(namesbefore[1],width=w,height=h,pointsize=14)
	plotMASingle(data.eSet,i,xlim=xlim,ylim=ylim,do.logtransform=FALSE)
	dev.off()
  
	png(namesbefore[2],width=wd,height=hg,pointsize=10)
	plotMASingle(data.eSet,i,xlim=xlim,ylim=ylim,do.logtransform=FALSE)
	dev.off()
	##### END[LATER]
